...
#pragma warning restore S2344 // Enumeration type names should not have "Flags" or "Enum" suffixes
	...
	public sealed class ProvideAutoLoadAttribute : RegistrationAttribute
	{
		...
		private readonly Guid loadGuid;
		...
		/// </summary>
		/// <param name="cmdUiContextGuid">Context which should trigger the loading of your package.</param>
		public ProvideAutoLoadAttribute(string cmdUiContextGuid, PackageAutoLoadFlags flags)
		{
		...
		private string RegKeyName
		{
			get
			{
				return string.Format(CultureInfo.InvariantCulture, "AutoLoadPackages\\{0}", loadGuid.ToString("B", CultureInfo.InvariantCulture));
			}
		}
